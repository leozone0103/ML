...
class BinaryLogReg:
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        regularizer = self._lambda * self.weight.dot(self.weight)
        return np.mean(np.log(1 / self.mu(X, y))) + regularizer
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        regularizer = 2 * self._lambda * self.weight
        return np.mean(((1 - self.mu(X, y)) * -y)[:,None] * X, axis=0) + regularizer
    ...
    def gradient_J_bias(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        return np.mean((1 - self.mu(X, y)) * -y, axis=0)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        res = self.bias + X.dot(self.weight)
        return np.sign(res)
    ...
    def step(self, X: np.ndarray, y: np.ndarray, learning_rate: float = 1e-4):
        ...
        Args:
            ...
            learning_rate (float, optional): Learning rate of SGD/GD algorithm.
                Defaults to 1e-4.
        """
        bias_d = learning_rate * self.gradient_J_bias(X, y)
        weight_d = learning_rate * self.gradient_J_weight(X, y)
        self.bias = self.bias - bias_d
        self.weight = self.weight - weight_d
    ...
    def train(
        ...
        if self.mode == "gd":
            ...
            for _ in range(epochs):
                ...
                result["test_errors"].append(self.misclassification_error(X_test, y_test))
                for _ in range(num_batches):
                    r = RNG.choice(n, batch_size)
                    self.step(X_train[r], y_train[r], learning_rate)
        return result
...
if __name__ == "__main__":
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Stochastic Gradient Descent Loss batch size=100")
    plt.show()
    ...
    plt.legend()
    plt.title("Stochastic Gradient Descent Misclassification Error batch size=100")
    plt.show()
